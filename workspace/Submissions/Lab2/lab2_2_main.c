...
//Added global variables to be used in new worker functions
...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer2, 200, 1000); //time interval was changed to 1ms
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    //passes every 100th time the function is called
	if ((CpuTimer2.InterruptCount % 100) == 0) {
		UARTPrint = 1;

        button = ReadSwitches();
        //if((GpioDataRegs.GPADAT.bit.GPIO5 == 0) & (GpioDataRegs.GPADAT.bit.GPIO6 == 0)){ //checks to see whether both buttons are pushed or not
        if((button & 0x6)== 0x6){
            numTimer2calls += 0; //halts numTimer2calls and stops the counter from updating
        }
        else{
            numTimer2calls++; //updates counter
        }

        SetLEDRowsOnOff(numTimer2calls); //takes the counter variable as a parameter so that the led lights count
	}
    x4 = x3 + 2.0;
    x3 = x4 + 1.3;
    x1 = 9*x2;
...
void SetLEDRowsOnOff(int16_t rows) {
    //lights up the specific rows based on the count being passed in the argument
    //rows++;
    if((rows & 0x1) == 0x1){
...
int16_t ReadSwitches(void){
    //reads whether the swtiches have been pushed or not and lights up those specific rows
    int16_t value = 0;
