...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer2, 200, 250000); //changed time to quarter seconds
...
//    init_serial(&SerialD,115200,serialRXD);
    ...
    {
        if (UARTPrint == 1 ) {
				serial_printf(&SerialA,"Num Timer2:%ld Num SerialRX: %ld Num Timer 2: %ld Button: %d \r\n",CpuTimer2.InterruptCount,numRXA, numTimer2calls, button);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer2_isr(void)
{
	//updates the counter variable every time the function is called
    numTimer2calls++;
    UARTPrint = 1;
	...
	//calls the ReadSwitches function on the button variable which is then passed in the SetLEDRowsOnOff function
	button = ReadSwitches();
	SetLEDRowsOnOff(button);
}
